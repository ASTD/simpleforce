...
func (obj *SObject) Upsert() (*SObject, error) {
	...
	if obj.Type() == "" || obj.client() == nil || obj.ExternalIDFieldName() == "" ||
		...
		return nil, errors.New(logPrefix + " required fields are missing. ExternalID, ExternalIDField and Type are required.")
	}
	...
	if err != nil {
		log.Println(logPrefix, "failed to convert sobject to json,", err)
		return nil, fmt.Errorf(logPrefix+" failed to convert sobject to json: %v", err)
	}
	...
	if err != nil {
		log.Println(logPrefix, "failed to process http request,", err)
		return nil, fmt.Errorf(logPrefix+" failed to process http request: %v", err)
	}
	...
	if len(respData) > 0 {
		...
		if err != nil {
			log.Println(logPrefix, "failed to parse response,", err)
			return nil, fmt.Errorf(logPrefix+" failed to parse response: %v", err)
		}
	}
